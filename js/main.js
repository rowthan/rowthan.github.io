jQuery(document).ready(function($) {
    ...
    $('#verticalTab').easyResponsiveTabs({//tab控制
        type: 'vertical',
        width: 'auto',
        fit: true
    });
    ...
    var animation_style = 'bounceInRight';//css动画效果 bounceIn、bounceInDown、bounceInRight、bounceInUp、bounceInLeft、fadeInDown、fadeInUp、fadeInLeft、fadeInRight、fadeIn、growIn、shake、shakeUp、rotateIn、rotateInUpLeft、rotateInDownLeft、rotateInUpRight、rotateInDownRight、rollIn、wiggle、swing、tada、wobble、pulse、lightSpeedInRight、lightSpeedInLeft、flip、flipInX、flipInY
    ...
    $('ul.resp-tabs-list li[class^=tabs-]').click(function() {
        ...
        if(tab_name == 'profile'){
            animation_style='bounceInDown';
        }
        if(tab_name == 'resume'){
            animation_style='bounceInUp';
        }
        if(tab_name == 'portfolio'){
            animation_style='bounceInRight';
        }
        if(tab_name == 'support'){
            animation_style='bounceInLeft';
        }
        if(tab_name == 'contact'){
            animation_style='bounceIn';
        }

        $('.resp-tabs-container').addClass('animated ' + animation_style);//.one 执行一次 事件绑定
    ...
    function redimensionnement() {//滚动条控制

        if (window.matchMedia("(max-width: 800px)").matches) {
            $(".content_2").mCustomScrollbar("destroy");//恢复到默认状态下
    ...
    // On lie l'événement resize à la fonction 窗口尺寸监听
    ...
    $('.clicklogo').click(function(){
        if(a){
            $(".clicklogo").animate({left:'50%'},"slow");
            a = false;
            $(".clickcontent").slideDown("slow");
            //$(".clickcontent").animate({left:'50%'},"slow");

        }
       else
        {
            $(".clicklogo").animate({left:'0'},"slow");
            a = true;
            $(".clickcontent").slideUp("slow");
        }
        //$(".clicklogo").animate({marginRight:'100%'},"slow");
            $(".clicktitle").toggle("slow");
        //$('.logo-top-margin').slideDown();
        //$('.logo-top-margin').animation();
        return false;
    });

    if ($.cookie('times') == null|| $.cookie('times')==0) {

        $('body').pagewalkthrough({
            name: 'introduction',
            steps: [
                {
                    popup: {content: '<span>欢迎来到我的网页版简历/个人网页设计作品。本网站由本人独自开发，采用网上开源框架和插件制作。本引导界面只运行一次！</span>', type: 'modal'}
                }, {
                    wrapper: '#verticalTab ul', popup: {content: '点击左侧进行切换内容。或点击音乐图标暂停/播放背景音乐', position: 'right'}
                }, {
                    wrapper: '#profile_info', popup: {content: '浏览过程中可以随时点击企鹅图标与我联系', position: 'bottom'}
                }
            ]
        });
        // Show the tour
